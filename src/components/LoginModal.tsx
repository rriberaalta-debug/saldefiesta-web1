...
const LoginModal: React.FC<LoginModalProps> = ({ onClose, onLogin, onGoogleSignIn, onSwitchToSignUp }) => {
  ...
  const handlePasswordReset = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.code === 'auth/user-not-found') {
          ...
          setError("Ocurrió un error al intentar enviar el email. Inténtalo de nuevo.");
      }
    } finally {
        setIsLoading(false);
    }
      ...
      <form onSubmit={handleLoginSubmit} className="space-y-4">
        ...
        <div className="relative">
          ...
          <input 
            ...
            required 
            className="w-full bg-gray-700 rounded-lg p-3 pl-10 pr-10 focus:outline-none focus:ring-2 focus:ring-festive-orange"
          />
  ...
  const renderResetView = () => (
    <>
      <h2 className="text-2xl sm:text-3xl font-bold mb-2 text-center">Restablecer Contraseña</h2>
      <p className="text-center text-gray-400 mb-6">Introduce tu email y te enviaremos un enlace para recuperarla.</p>
       {error && <p className="text-red-400 bg-red-900/50 p-3 rounded-lg text-sm text-center mb-4">{error}</p>}
      <form onSubmit={handlePasswordReset} className="space-y-4">
        <div className="relative">
          <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
          <input type="email" placeholder="Email" value={resetEmail} onChange={e => setResetEmail(e.target.value)} required className="w-full bg-gray-700 rounded-lg p-3 pl-10 focus:outline-none focus:ring-2 focus:ring-festive-orange" autoFocus/>
        </div>
        <button type="submit" disabled={isLoading} className="w-full bg-sky-500 font-bold py-3 rounded-lg hover:bg-sky-600 transition-colors disabled:opacity-50">
          {isLoading ? 'Enviando...' : 'Enviar Enlace'}
        </button>
      </form>
      <p className="text-center text-sm text-gray-400 mt-6">
        <button onClick={() => { setView('login'); setError(''); setMessage(''); }} className="font-semibold text-festive-orange hover:underline">
  ...
  const renderResetSentView = () => (
     <>
      <h2 className="text-2xl sm:text-3xl font-bold mb-2 text-center">Revisa tu Email</h2>
      <p className="text-center text-green-300 bg-green-900/50 p-4 rounded-lg my-6">{message}</p>
      <p className="text-center text-sm text-gray-400 mt-6">
        <button onClick={() => { setView('login'); setError(''); setMessage(''); }} className="font-semibold text-festive-orange hover:underline">
          Volver a Iniciar Sesión
        </button>
